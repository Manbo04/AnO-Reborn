...
def generate_province_revenue_optimized():
    ...
    with get_db_connection() as conn:
        ...
        dbdict.execute("""
            SELECT proInfra.id, provinces.userId, provinces.land, provinces.productivity 
            FROM proInfra 
        ...
        dbdict.execute("SELECT user_id, * FROM upgrades WHERE user_id = ANY(%s)", (user_ids,))
        ...
        dbdict.execute("SELECT user_id, education FROM policies WHERE user_id = ANY(%s)", (user_ids,))
        ...
        money_updates = []          # (amount, user_id)
        energy_updates = []         # (energy, province_id)
        province_field_updates = {} # province_id -> {field -> value}
        resource_updates = {}       # user_id -> {resource -> value}
        
        # For large batch operations
        stat_gold_updates = []      # (gold_amount, user_id)
        ...
        for infra_row in infra_records:
            ...
            if user_id not in resource_updates:
                resource_updates[user_id] = dict(user_resources)  # Start with current state
            ...
            for unit in columns:
                ...
                productivity_multiplier = 1.0 + ((productivity - 50) * variables.DEFAULT_PRODUCTIVITY_PRODUCTION_MUTLIPLIER)
                ...
                if current_money < operating_costs:
                    has_enough = False
                    log_verbose(f"Skip {unit} province {province_id}: not enough money ({current_money} < {operating_costs})")
                    continue
                ...
                if unit in energy_consumers:
                    current_energy = province_field_updates[province_id].get('energy', 0)
                    new_energy = current_energy - unit_amount
                    if new_energy < 0:
                        has_enough = False
                        log_verbose(f"Skip {unit} province {province_id}: not enough energy")
                        continue
                ...
                for resource, amount in minus_config.items():
                    amount *= unit_amount
                    if unit == "component_factories" and upgrades.get("automationintegration"):
                        amount *= 0.75
                ...
                for resource, amount in plus_config.items():
                    amount = (amount + plus_amount) * unit_amount * plus_amount_multiplier
                    amount = math.ceil(amount)

                    if resource in percentage_based:
                        # Province-level resource (percentage-based)
                        current_val = province_field_updates[province_id].get(resource, 0)
                        new_val = min(100, current_val + amount)
                        province_field_updates[province_id][resource] = new_val
                        log_verbose(f"Province {province_id}: +{amount} {resource} -> {new_val}")
                    else:
                        # User-level resource
            ...
            # ===== POLICY MODIFIERS =====
            if 5 in policies:
                current_prod = province_field_updates[province_id].get('productivity', 50)
                province_field_updates[province_id]['productivity'] = max(0, int(current_prod * 0.91))
            if 4 in policies:
                current_prod = province_field_updates[province_id].get('productivity', 50)
                province_field_updates[province_id]['productivity'] = min(100, int(current_prod * 1.05))
            if 2 in policies:
                current_happy = province_field_updates[province_id].get('happiness', 50)
                province_field_updates[province_id]['happiness'] = max(0, int(current_happy * 0.89))
