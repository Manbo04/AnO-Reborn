...
def test_reset_password_invalid_code_shows_error(client):
    ...
    assert resp.status_code == 400
    assert (
        b"Invalid or expired reset code" in resp.data
        or b"Invalid or expired reset code." in resp.data
    )
...
def test_countries_pagination_preserves_filters(client):
    ...
    # The form should reflect the requested filters
    assert 'value="testsearch"' in data
    assert (
        '<option value="population"' in data
        and "selected"
        in data.split('<option value="population"')[1].split("</option>")[0]
    )
