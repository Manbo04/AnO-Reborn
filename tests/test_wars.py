...
def users():
    users = [
        {'username': 'waruser1', 'email': 'waruser1@example.com', 'password': 'testpass', 'confirmation': 'testpass', 'key': 'key1', 'continent': 'Europe'},
        {'username': 'waruser2', 'email': 'waruser2@example.com', 'password': 'testpass', 'confirmation': 'testpass', 'key': 'key2', 'continent': 'Asia'}
    ]
    ...
    for user in users:
        db.execute("DELETE FROM stats WHERE id IN (SELECT id FROM users WHERE username=%s)", (user['username'],))
        db.execute("DELETE FROM military WHERE id IN (SELECT id FROM users WHERE username=%s)", (user['username'],))
        db.execute("DELETE FROM resources WHERE id IN (SELECT id FROM users WHERE username=%s)", (user['username'],))
        db.execute("DELETE FROM upgrades WHERE user_id IN (SELECT id FROM users WHERE username=%s)", (user['username'],))
        db.execute("DELETE FROM policies WHERE user_id IN (SELECT id FROM users WHERE username=%s)", (user['username'],))
    ...
    for user in users:
        db.execute("INSERT INTO keys (key) VALUES (%s) ON CONFLICT DO NOTHING", (user['key'],))
        # Hash password as in signup.py
        hashed = bcrypt.hashpw(user['password'].encode('utf-8'), bcrypt.gensalt(14)).decode('utf-8')
        db.execute("INSERT INTO users (username, email, date, hash, auth_type) VALUES (%s, %s, %s, %s, %s)",
               (user['username'], user['email'], str(datetime.date.today()), hashed, 'normal'))
        db.execute("SELECT id FROM users WHERE username = (%s)", (user['username'],))
        user_id = db.fetchone()[0]
        db.execute("INSERT INTO stats (id, location) VALUES (%s, %s)", (user_id, user['continent']))
...
def test_declare_war(login_session, users):
    ...
    db.execute("SELECT * FROM wars WHERE attacker=(SELECT id FROM users WHERE username=%s) AND defender=%s AND peace_date IS NULL", (users[0]['username'], defender_id))
    war = db.fetchone()
...
def test_peace_offer(login_session, users):
    ...
    data = {'money': '100'}
    r = login_session.post(f"{BASE_URL}/send_peace_offer/{war_id}/{enemy_id}", data=data, allow_redirects=True)
    assert r.status_code == 200 or r.status_code == 302
