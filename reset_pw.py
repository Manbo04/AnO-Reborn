...
if not new_password:
    logging.getLogger(__name__).error(
        "No new password specified. Set RESET_PW_NEW_PASSWORD env var to proceed."
    )
    raise SystemExit(1)
...
try:
    logging.getLogger(__name__).info(
        "Attempting to connect to %s:%s/%s...",
        db_config["host"],
        db_config["port"],
        db_config["database"],
    )
    ...
    if not user:
        ...
        logging.getLogger(__name__).info(
            "Password reset successful for user %s (ID: %s)", username, user_id
        )
