...
class Client(object):
    ...
    def _make_request(self, opener, request, timeout=None):
        ...
        try:
            ...
            exc.__cause__ = None
            _logger.debug(
                "{method} Response: {status} {body}".format(
                    method=request.get_method(), status=exc.status_code, body=exc.body
                )
            )
            raise exc
            ...
            def http_request(
                ...
                **_
                ...
                if request_body is None:
                    ...
                    if (
                        "Content-Type" in self.request_headers
                        and self.request_headers["Content-Type"] != "application/json"
                    ):
                ...
                _logger.debug(
                    "{method} Request: {url}".format(
                        method=method, url=request.get_full_url()
                    )
                )
                if request.data:
                    _logger.debug("PAYLOAD: {data}".format(data=request.data))
                _logger.debug("HEADERS: {headers}".format(headers=request.headers))
                ...
                _logger.debug(
                    "{method} Response: {status} {body}".format(
                        method=method, status=response.status_code, body=response.body
                    )
                )
