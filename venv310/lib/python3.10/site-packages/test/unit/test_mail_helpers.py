...
from sendgrid.helpers.mail import (Asm, Attachment, Bcc, Cc, ClickTracking,
                                   Content, DynamicTemplateData, Email, From,
                                   Mail, Personalization, Subject,
                                   Substitution, To, TrackingSettings)
...
response_content_with_all_three_mime_contents = json.dumps(
    {
        "content": [
            ...
            {"type": "text/x-amp-html", "value": amp_html_content},
            {
                "type": "text/html",
                "value": "<strong>and easy to do anywhere, even with Python</strong>",
            },
        ],
        "from": {"email": "test+from@example.com", "name": "Example From Name"},
        "personalizations": [
            {"to": [{"email": "test+to@example.com", "name": "Example To Name"}]}
        ],
...
class UnitTests(unittest.TestCase):
    def test_asm(self):
        ...
        asm2 = Asm(1, [1, 2, 3])
        self.assertEqual(asm1.group_id.get(), asm2.group_id.get())
        self.assertEqual(asm1.groups_to_display.get(), asm2.groups_to_display.get())

    def test_attachment(self):
        from sendgrid.helpers.mail import (ContentId, Disposition, FileContent,
                                           FileName, FileType)
        ...
        a2 = Attachment(
            "Base64EncodedString", "example.pdf", "application/pdf", "attachment", "123"
        )
    ...
    def test_single_email_to_a_single_recipient(self):
        from sendgrid.helpers.mail import (From, HtmlContent, Mail,
                                           PlainTextContent, Subject, To)
    ...
    def test_single_email_with_all_three_email_contents_to_single_recipient(self):
        from sendgrid.helpers.mail import (AmpHtmlContent, From, HtmlContent,
                                           Mail, PlainTextContent, Subject, To)
        ...
        response_content = json.dumps(
            {
                ...
                "personalizations": [
                    {
                        "to": [
                            {"email": "test+to@example.com", "name": "Example To Name"}
                        ]
                    }
                ],
                "subject": "Sending with SendGrid is Fun",
    ...
    def test_single_email_with_all_three_contents_in_collapsed_order_of_plain_amp_html_content_single_recipient(
        self,
    ):
        from sendgrid.helpers.mail import (AmpHtmlContent, From, HtmlContent,
                                           Mail, PlainTextContent, Subject, To)
    ...
    def test_single_email_with_all_three_contents_in_collapsed_order_of_html_plain_amp_content_single_recipient(
        self,
    ):
        from sendgrid.helpers.mail import (AmpHtmlContent, From, HtmlContent,
                                           Mail, PlainTextContent, Subject, To)
    ...
    def test_single_email_with_all_three_contents_in_collapsed_order_of_html_amp_plain_content_single_recipient(
        self,
    ):
        from sendgrid.helpers.mail import (AmpHtmlContent, From, HtmlContent,
                                           Mail, PlainTextContent, Subject, To)
    ...
    def test_single_email_with_all_three_contents_in_collapsed_order_of_amp_html_plain_content_single_recipient(
        self,
    ):
        from sendgrid.helpers.mail import (AmpHtmlContent, From, HtmlContent,
                                           Mail, PlainTextContent, Subject, To)
    ...
    def test_personalization_add_email_filters_out_duplicate_to_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
    ...
    def test_personalization_tos_setter_filters_out_duplicate_dict_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
    ...
    def test_personalization_tos_setter_filters_out_duplicate_to_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
    ...
    def test_personalization_ccs_setter_filters_out_duplicate_dict_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
        ...
        cc_email_with_caps = {
            "email": "test+CC0@example.com",
            "name": "Example Cc Name 0",
        }
    ...
    def test_personalization_ccs_setter_filters_out_duplicate_cc_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
    ...
    def test_personalization_bccs_setter_filters_out_duplicate_dict_emails(self):
        ...
        p = Personalization()
        bcc_emails = [
            {"email": "test+bcc0@example.com", "name": "Example Bcc Name 0"}
        ] * 2
        p.bccs = bcc_emails

        self.assertEqual([bcc_emails[0]], p.bccs)

    def test_personalization_bccs_setter_filters_out_duplicate_dict_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
        ...
        bcc_email_with_caps = {
            "email": "test+BCC0@example.com",
            "name": "Example Bcc Name 0",
        }
    ...
    def test_personalization_bccs_setter_filters_out_duplicate_bcc_emails_ignoring_case(
        self,
    ):
        self.maxDiff = None
    ...
    def test_kitchen_sink(self):
        from sendgrid.helpers.mail import (Asm, Attachment, BatchId, Bcc,
                                           BccSettings, BccSettingsEmail,
                                           BypassBounceManagement,
                                           BypassListManagement,
                                           BypassSpamManagement,
                                           BypassUnsubscribeManagement,
                                           Category, Cc, ClickTracking,
                                           Content, ContentId, CustomArg,
                                           Disposition, FileContent, FileName,
                                           FileType, FooterHtml,
                                           FooterSettings, FooterText, From,
                                           Ganalytics, GroupId,
                                           GroupsToDisplay, Header, IpPoolName,
                                           Mail, MailSettings, MimeType,
                                           OpenTracking,
                                           OpenTrackingSubstitutionTag,
                                           ReplyTo, SandBoxMode, Section,
                                           SendAt, SpamCheck, SpamThreshold,
                                           SpamUrl, Subject, SubscriptionHtml,
                                           SubscriptionSubstitutionTag,
                                           SubscriptionText,
                                           SubscriptionTracking, Substitution,
                                           TemplateId, To, TrackingSettings,
        ...
        message.content = Content(
            MimeType.text, "and easy to do anywhere, even with Python"
        )
        message.content = Content(
            MimeType.html, "<strong>and easy to do anywhere, even with Python</strong>"
        )
        message.content = [
    ...
    def test_unicode_values_in_substitutions_helper(self):
        ...
        message.substitution = Substitution("%city%", "Αθήνα", p=1)
        self.assertEqual(
            message.get(),
            json.loads(
                r"""{
    ...
    def test_v3_bypass_filters(self):
        from sendgrid.helpers.mail import (BypassBounceManagement,
                                           BypassSpamManagement,
                                           BypassUnsubscribeManagement,
                                           MailSettings)
