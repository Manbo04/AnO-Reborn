...
with get_db_connection() as conn:
    db = conn.cursor()
    # Ensure canonical account exists and reuse/create a province for it (no destructive deletes)
    db.execute(
...
# attempt to find error message in the page
import re

m = re.search(r"<h1>\s*(\d+)\s*<\/h1>\s*<p>(.*?)<\/p>", html, re.S)
if m:
    print("error code in page", m.group(1))
    print("message", m.group(2)[:200])
else:
    # fallback search
    for s in ["You don't have", "Missing", "Trade", "error", "not found"]:
        if s in html:
            print("found marker", s)
print("---HTML---")
print(html)
