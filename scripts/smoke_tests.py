...
def check_countries():
    with app.test_client() as c:
        resp = c.get(
            "/countries?"
            + urlencode(
                {"sort": "population", "sortway": "asc", "search": "testsearch"}
            )
        )
        data = resp.get_data(as_text=True)
        print("countries GET status:", resp.status_code)
        print(
            "population selected present:",
            'value="population"' in data
            and "selected" in data.split('value="population"')[1][:50],
        )
        print(
            "sortway asc selected present:",
            'value="asc"' in data and "selected" in data.split('value="asc"')[1][:50],
        )
...
def check_coalitions():
    with app.test_client() as c:
        resp = c.get(
            "/coalitions?"
            + urlencode({"sort": "open", "sortway": "asc", "search": "coaltest"})
        )
        data = resp.get_data(as_text=True)
        print("\ncoalitions GET status:", resp.status_code)
        print(
            "open selected present:",
            'value="open"' in data and "selected" in data.split('value="open"')[1][:50],
        )
        print(
            "sortway asc selected present:",
            'value="asc"' in data and "selected" in data.split('value="asc"')[1][:50],
...
def check_password_reset():
    with app.test_client() as c:
        ...
        print(
            "\nrequest_password_reset nonexistent email status:",
            resp.status_code,
            "location:",
