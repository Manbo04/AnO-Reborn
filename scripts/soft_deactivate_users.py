...
def run(dry_run=True, exclude_admins=True):
    ...
    with get_db_connection() as conn:
        cur = conn.cursor()
        cur.execute("SELECT id, username, email FROM users WHERE is_active IS DISTINCT FROM FALSE" + exclude_clause)
        rows = cur.fetchall()
        ...
        cur.execute("UPDATE users SET is_active = FALSE, reset_required = TRUE, reset_at = %s WHERE is_active IS DISTINCT FROM FALSE" + exclude_clause, (now,))
        conn.commit()
