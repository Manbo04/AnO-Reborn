...
**1. The CSS bundle system — edit the right file or your change silently vanishes, AND it silently never reaches production either unless you also confirm the deploy actually regenerates it.**
`scripts/bundle_game_css.py` appends a fixed list of files (`FILES` at the top of that script — every new `static/css/*.css` file must be added there by hand, it does not scan the directory) onto the end of `static/style.css`, after a marker comment (`/* === GAME UI BUNDLE (auto-generated) === */`, currently ~line 5569). Anything in `static/style.css` **after** that marker is auto-generated and gets overwritten on the next bundle run. Before editing `static/style.css`, check which side of the marker your target line is on:
- Before the marker → hand-written, edit directly.
- After the marker → find the real source in `static/css/<file>.css` and edit that instead (and make sure it's listed in `FILES`).

**Critical, found the hard way on 2026-08-27: `nixpacks.toml`'s `[phases.build]` step (`python3 scripts/bundle_game_css.py`) is dead config.** Railway actually builds this repo from the root `Dockerfile`, not nixpacks — confirmed via `railway logs --build`, which shows a Docker multi-stage build (`FROM python:3.10-slim as builder` / `stage-1`), not a nixpacks build. The Dockerfile's `COPY . .` just copies whatever `static/style.css` was last **committed**, verbatim — no bundling ever ran. This meant CSS source edits could be committed, pushed, and deployed successfully (`/deploy-info` showing the right commit, `railway status` showing `SUCCESS`) while the actual served CSS stayed stale indefinitely — confirmed live via `static/style.css`'s `last-modified` response header staying frozen across multiple real deploys. Fixed by adding `RUN python3 scripts/bundle_game_css.py` to the Dockerfile right after `COPY . .`, so bundling now happens fresh on every build. **This should mean future CSS source edits work correctly without manual intervention — but if a CSS fix ever again doesn't appear live after a successful deploy, check `static/style.css`'s `Last-Modified` header against the deploy time before assuming the CSS itself is wrong; that symptom means the bundling step isn't running, not that the fix is bad.**
...
**7. `static/style.min.css`, if present, silently wins over `static/style.css` for local verification.** `game_ui.py`'s `game_stylesheet_filename()` prefers the minified file whenever it exists on disk. It's untracked (never committed) and production regenerates it fresh at build time via the `Dockerfile`'s `RUN python3 scripts/bundle_game_css.py` step (see landmine #1 — this used to be nixpacks.toml-only dead config and silently didn't run at all until 2026-08-27), so this is harmless in production — but a locally-stale copy will make your local dev server silently serve old CSS while you edit `style.css`/`static/css/*.css`, making a real fix look like it isn't working. If a local CSS check doesn't reflect an edit you just made, run `python3 scripts/bundle_game_css.py` before assuming the fix is wrong.
...
| `account.html` | **Resolved, 2026-08-31** | `game_glass_panel`, `game_hero` (in the new `account_v2.html`, behind `THEME_V2_PAGES`) | Both pre-existing bugs fixed directly on the still-live `account.html`, independent of the v2 port: the orphaned `min-width: 120px;` + stray `}` fragment removed; the duplicate `.account-action-form` selector consolidated into the one actually winning the cascade (net computed style unchanged, verified). The delete/logout confirmation modal's decision is made: **theme-aware**, not a deliberately distinct treatment — swapped its hardcoded dark colors for real tokens (`--foreground`/`--border`/`--colorOne`/`--text-secondary`/`--background`); dark theme looks the same as before (token values are near-identical to the old hardcoded ones), light theme now actually renders correctly. See `/Users/dede/ano-redesign/plan/PRODUCTION_PORT_PLAN.md` §13. |
| `assembly.html` | **Resolved, 2026-08-31 — intentional, not touched** | — | The design-direction call is made: **intentional STYLE_BIBLE "Vivid" moment, stays theme-ignoring by design**. A fully independent local mockup pass (built without reference to this tracker) landed on the exact same bespoke, non-token-following, always-dark glassmorphic treatment for this page's voting UI — two separate design passes converging on the same answer is a real signal, not a coincidence to second-guess. No CSS changes made. Also functionally re-verified (real render, not just reading): the real `/assembly` route's vote-tally math, the empty "no votes yet" state (no division-by-zero, no accidental pre-checked radio), and the "YOUR VOTE" badge logic all render correctly — no hidden bugs found on this page, unlike several others checked this same session. See `/Users/dede/ano-redesign/plan/PRODUCTION_PORT_PLAN.md` §13. |
